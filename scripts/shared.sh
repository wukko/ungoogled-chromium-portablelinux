...
fetch_sources() {
    ...
    if ${use_clone}; then
        ...
        "${_main_repo}/utils/clone.py" --sysroot "$_host_arch" -o "${_src_dir}"
    else
...
fix_tool_downloading() {
    ...
    sed -i 's/chromium.9oo91esource.qjz9zk/chromium.googlesource.com/g' "${_src_dir}/tools/clang/scripts/build.py"
}

setup_toolchain() {
    "${_src_dir}/tools/rust/update_rust.py"

    # Chromium currently has no non-x86 llvm builds on
